...
// 修改：增加 page 和 context，Playwright 开发中 page 使用频率最高
...
async function fastopen(filter?: string | FilterOptions): Promise<OpenResult[]> {
  ...
  const results = await Promise.all(res.map(v => queue(() => open(v.id).catch(e => console.error(`[Skip] ${v.id} 启动失败: ${(e as Error).message}`)))));
  return results.filter((item): item is OpenResult => item != null);
}

/**
 * 批量筛选关闭
 */
async function fastclose(filter: FilterOptions = {}): Promise<any[]> {
 ...
 * 根据选项删除浏览器配置
 */
async function delopt(option: FilterOptions = {}): Promise<any> {
  try {
 ...
 */
async function delgroup(groupName: string): Promise<any> {
  if (!groupName) {
 ...
 */
async function delname(name: string): Promise<any> {
  if (!name) {
 ...
 * @returns 浏览器id
 */
async function create(options: CreateProfile = {}): Promise<string> {
  const json = {
    ...{
      groupId: null,
      name: '',
  ...
  const res: any = await kit.req('http://127.0.0.1:54345/browser/update post', {
    json,
  });
...
async function open(id: string | BrowserProfile): Promise<OpenResult> {
  ...
  // 调用比特浏览器 API 打开窗口
  let res: any = await kit.req('http://127.0.0.1:54345/browser/open post', {
    json: {
      id: browserId,
      args: [], // 你的自定义 args
...
async function list(filterOptions: FilterOptions = {}): Promise<BrowserProfile[]> {
  ...
  while (true) {
    try {
      const res: any = await kit.req('http://127.0.0.1:54345/browser/list/concise post', {
        json: {
          page: currentPage,
          pageSize: pageSize,
        },
      ...
      if (res.data && res.data.success && res.data.data && res.data.data.list) {
        ...
        if (profilesOnPage.length === 0) break;
        allProfiles.push(...profilesOnPage);
        if (profilesOnPage.length < pageSize) break;
        currentPage++;
      } else {
...
async function close(id: string): Promise<any> {
  const res: any = await kit.req('http://127.0.0.1:54345/browser/close post', {
    json: {id},
  });
  return res.data;
}
...
async function del(ids: string[]): Promise<any> {
  const res: any = await kit.req('http://127.0.0.1:54345/browser/delete/ids post', {
    json: {ids},
  });
  return res.data;
}

async function delall(): Promise<any> {
